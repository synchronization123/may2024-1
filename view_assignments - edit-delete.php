...
// Exception and error handling
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_assignment'])) {
        ...
        $query = "UPDATE patches SET title = :title, description = :description, assigned_to = :assigned_to, product_name = :product_name, rm_pm = :rm_pm WHERE assignment_id = :assignment_id";
        $stmt = $db->prepare($query);
