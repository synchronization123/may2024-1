...
try {
    ...
    $user_id = $_SESSION['user_id'];
    $query = "SELECT a.assignment_id, a.assigned_date, a.title, a.aging, CONCAT(u.ufname, ' ', u.ulname) AS assigned_to, a.status, a.review_status, a.certification_status, a.security_jira, a.functional_jira, a.koc_date, a.eta 
              FROM patches a
...
<div class="form-group">
                ...
                <tr>
                    ...
                    <th>Status 
						...
						<option value="All">All</option>
                            ...
                            <option value="InProgress" selected>Open</option>
					...
					<th> <center>Certification Status</center>
                        <select id="filter_certstatus" >
						<option value="All">All</option>
					...
					<th> <center>Review Status</center>
                        <select id="filter_reviewstatus" >
						<option value="All">All</option>
                            <option value="Passed">Passed</option>
                            <option value="Passed with Exception">Passed with Exception</option>
    ...
    <?php foreach ($assignments as $assignment) : ?>
        <tr>
			...
			<td>
                                    <select class="form-control status-dropdown" data-id="<?= $assignment['assignment_id'] ?>" data-user="<?= $user_id ?>">

                                    <option value="Pending" <?= ($assignment['certification_status'] == 'Pending') ? 'selected' : '' ?>>Pending</option>
									 <option value="Approved" <?= ($assignment['certification_status'] == 'Approved') ? 'selected' : '' ?>>Approved</option>
                                    <option value="Approved with Exception" <?= ($assignment['certification_status'] == 'Approved with Exception') ? 'selected' : '' ?>>Approved with Exception</option>
                                    <option value="Rejected" <?= ($assignment['certification_status'] == 'Rejected') ? 'selected' : '' ?>>Rejected</option>
                                </select>
                            </td>
							...
							<td>
                                    <select class="form-control status-dropdown1" data-id="<?= $assignment['assignment_id'] ?>" data-user="<?= $user_id ?>">

                                    <option value="Pending" <?= ($assignment['review_status'] == 'Pending') ? 'selected' : '' ?>>Pending</option>
									 <option value="Passed" <?= ($assignment['review_status'] == 'Passed') ? 'selected' : '' ?>>Passed</option>
                                    <option value="Passed with Exception" <?= ($assignment['review_status'] == 'Passed with Exception') ? 'selected' : '' ?>>Passed with Exception</option>
                                    <option value="Failed" <?= ($assignment['review_status'] == 'Failed') ? 'selected' : '' ?>>Failed</option>
                                </select>
                            </td>
...
// Event listener for status filter
document.getElementById("filter_certstatus").addEventListener("change", function () {
    filterbycertstatus([this.value]);
});
 ...
 <script>
        ...
        $('.status-dropdown').change(function(){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    // AJAX request to insert user_id into lead_username
                    $.ajax({
                        url: 'insert_lead_username.php',
                        method: 'POST',
                        data: {assignment_id: assignmentId, user_id: userId},
                        success: function(response){
                            // Show success message or perform any other action
                            console.log(response);
                        }
    ...
    $(document).ready(function(){
        // When status dropdown value changes
        $('.status-dropdown1').change(function(){
            var assignmentId = $(this).data('id');
            var userId = $(this).data('user');
            var status = $(this).val();
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {assignment_id: assignmentId, review_status: status},
                success: function(response){
                    // Show success message or perform any other action
                    console.log(response);
