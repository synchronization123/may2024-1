...
try {
    ...
    // Fetch assignments_vapt data (excluding soft-deleted assignments_vapt) assigned to the current session user
    $user_id = $_SESSION['user_id'];
    $query = "SELECT a.assignment_id, a.assigned_date, a.title, a.description, CONCAT(u.ufname, ' ', u.ulname) AS assigned_to, a.product_name, a.rm_pm 
              FROM assignments_vapt a
              JOIN users u ON a.assigned_to = u.user_id
              WHERE a.delete_flag = 0 AND a.assigned_to = :user_id";

    $stmt = $db->prepare($query);
    $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt->execute();
    $assignments_vapt = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>View assignments_vapt</title>
